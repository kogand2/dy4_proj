...
if __name__ == "__main__":
    ...
    #RF front end stops here==================================================
		...
		samplesQ, manchester_values, cdr_stateQ, initialQ = CDR_state(demod_filtQ, sps, cdr_stateQ, initialI)
		...
		if block_count >= 3 and block_count < 6:
			...
			axs[0].plot(samplesI, np.zeros(shape = len(samplesI)), marker="o")
			#print(manchester_values)
			axs[0].set_title('', fontstyle='italic',fontsize='medium')
			axs[0].axhline(y = 0, color = 'r', linestyle = '-')

			axs[1].plot(range(len(demod_filt)), demod_filt, c='blue')
			#axs[0].plot(x1, prev1[(len(prev1)-n):], c='orange')
			#axs[0].plot(sampling_intervals, np.zeros(shape = len(sampling_intervals)), marker="x", c='orange', markersize=15)
			axs[1].plot(samplesQ, np.zeros(shape = len(samplesQ)), marker="o")
			#print(manchester_values)
			axs[1].set_title('', fontstyle='italic',fontsize='medium')
